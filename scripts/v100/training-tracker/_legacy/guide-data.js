...
const GUIDE_CHAPTERS = [
    {
        ...
        sections: [
            ...
            },
            {
                title: '训练流程总览',
                type: 'text',
                content: '环境安装 → 数据准备 → 检索服务 → 开始训练 → Rollout → 训练监控 → 模型评估 → 模型部署'
            },
            {
                title: '两条训练路线',
                type: 'table',
    ...
    },
    {
        ...
        sections: [
            ...
            },
            {
                ...
                type: 'code',
                lang: 'bash',
                content: 'curl -s http://127.0.0.1:8000/retrieve \\\n  -X POST \\\n  -H "Content-Type: application/json" \\\n  -d \'{"query": "What is the capital of France?", "topk": 3}\' | python -m json.tool'
            },
            {
                title: '检索服务架构',
                type: 'text',
                content: '训练进程 (8 GPU) → HTTP POST /retrieve → 检索服务 (1 GPU) → e5-base-v2 编码查询 → faiss-gpu 向量检索 → 返回 top-3 文档'
            },
            {
                title: '注意事项',
                type: 'warning',
                content: '检索服务和训练共享 GPU 0。训练脚本的 gpu_memory_utilization 已调低给检索服务留空间。如果 OOM，可以把检索服务移到 CPU 模式启动（去掉 --faiss_gpu）。'
            }
        ]
    },
    {
    ...
    },
    {
        ...
        sections: [
            ...
            {
                title: '评测目标（Search-R1 Qwen2.5-3B）',
                type: 'table',
                headers: ['数据集', 'Search-R1 原始', 'verl AgentLoop 复现', '提升'],
                rows: [
                    ['NQ', '0.341', '0.4640', '+36.1%'],
                    ['TriviaQA', '0.545', '0.6164', '+13.1%'],
                    ['PopQA', '0.378', '0.4239', '+12.1%'],
                    ['HotpotQA', '0.324', '0.4225', '+30.4%'],
                    ['2Wiki', '0.319', '0.3979', '+24.7%'],
                    ['Musique', '0.103', '0.1808', '+75.5%'],
                    ['Bamboogle', '0.264', '0.3440', '+30.3%'],
                    ['平均', '0.325', '0.4071', '+25.3%'],
                ]
            },
            {
                title: '评测通过标准',
        ...
        sections: [
            {
                title: '说明',
                type: 'tip',
                content: '详见 PERFORMANCE_GUIDE.md，这里列出最常见的性能问题。'
            },
            {
                title: '常见性能瓶颈',
                type: 'table',
                headers: ['现象', '可能原因', '解决方案'],
                rows: [
                    ['Rollout 很慢', '检索服务响应慢', '检查检索服务日志，考虑 GPU 模式'],
                    ['GPU 利用率低', 'CPU offload 瓶颈', '确认内存充足，减少 offload 层数'],
                    ['OOM', 'max_model_len 过大', '降低到 12000-15000'],
                    ['训练不收敛', '学习率不合适', '尝试 5e-7 到 2e-6 范围'],
                ]
            }
        ]
    },
    {
        ...
        sections: [
            ...
            },
            {
                ...
                type: 'list',
                items: [
                    '训练初期正常，如果持续 100+ 步还是这样：',
                    '检查 tool_call_success_rate 指标',
                    '确认 attn_implementation=sdpa 设置正确',
                    '尝试降低学习率',
                ]
            },
            {
